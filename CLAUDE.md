...
- `appendLead`/`appendOutbound` (`src/lib/admin/store.ts`) are intentionally no-ops, and `readLeads`/`readOutbound` always return `[]` — there is no KV/R2/D1 behind them. Every form submission is only ever delivered as an email via Resend (`src/lib/email.ts`); nothing is persisted server-side.
