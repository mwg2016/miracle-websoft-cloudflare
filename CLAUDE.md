...
npx wrangler r2 bucket create miracle-websoft-resumes
...
```
Local dev (`npm run dev`) proxies real KV/R2 bindings via miniflare — see `initOpenNextCloudflareForDev()` at the bottom of `next.config.ts`.
...
## Admin lead/outbound/resume storage
...
- Resume uploads go to the `RESUMES_R2` R2 bucket, keyed `<leadId>_<sanitizedFilename>`.
...
- Local dev and production share the same KV namespace/R2 bucket by default (unlike the old `data/dev` vs `data/prod` filesystem split) — if you want isolation, create a second KV namespace/R2 bucket and point `wrangler.jsonc`'s dev environment at it.
